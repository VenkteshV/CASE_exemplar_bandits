...
"""
api_keys = ["sk-f8946b0837ae469a886d650b348f2ad2"]
endpoint_urls = ["https://68d1-130-75-152-24.ngrok.app"]
llm_names = []
...
class top_m_arm:
    ...
    def compute_Jt(self):
        ...
        if(self.t==0):
            ...
            equal_prev = [True if v in self.previous_J else False for v in self.J]
            increase_patience = all(equal_prev)
            if increase_patience:
...
def static_subset_selection(val_data, train_data, k, test_data, train_emb_dict, test_emb_dict):
    ...
    for r, i in train_data.iterrows():
        train_emb.append(train_emb_dict[str(r)])
    ...
    # ######### Making feature matrix of dim = (num_of_arms, num_of_val_data).T
    # len_L = len(L)
    # L_val = np.zeros((len_L,len(val_emb)))
    # for u in range(len_L):
    #     avg = np.zeros((1, len(val_emb)))
 ...
 # E = (num of ices * num of val samples)
 # W = (num of ices *val samples)

    # ########### Making feature matrix of dim = (num_of_arms, num_of_ices).T
    len_L = len(L)
    ...
    folder1 = f"loss_folder_tabmwp_llama2"
...
def get_open_source_completions(test_data, data):
    ...
    with open('../data/tabmwp/tabmwp_test_emb.pkl', 'rb') as f:
        te = pickle.load(f)
    
    with open('../data/tabmwp/tabmwp_train_emb.pkl', 'rb') as f1:
        tr = pickle.load(f1)
    
    # Map embeddings from dataset ----- json to dict
    k = 0
    dev_set1 = open("../data/tabmwp/tabmwp_dev.json")
    ...
    train_set1 = open("../data/tabmwp/tabmwp_train.json")
    ...
    merged_exemplars.to_csv("static_subset_selection_llama2_tabmwp.csv")
    ...
    index=0
    acc_records = []
    selected_indices = [7,617,1333,2868,4944]
    #exemplars = data.iloc[selected_indices]
    ...
    for index, row in test_data.iterrows():
        ...
        final_questions.to_csv("tabwmp_llama2_case.tsv",sep="\t",index=False)
...
def test_few_shot_prediction():
    ...
    train_data = pd.read_json('../data/tabmwp/problems_train.json', orient='index')
    test_data = pd.read_json('../data/tabmwp/problems_dev.json', orient='index')
